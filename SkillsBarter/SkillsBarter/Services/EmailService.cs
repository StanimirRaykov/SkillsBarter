...
public class EmailService : IEmailService
{
    ...
    )
    {
        try
        {
            ...
            var verificationUrl =
                $"{appUrl}/verify-email?token={Uri.EscapeDataString(verificationToken)}";
    ...
    )
    {
        try
        {
            ...
            var message = new EmailMessage
            {
                ...
                TextBody =
                    $@"Hello {userName}, We received a request to reset your password for your SkillsBarter account. Click the link below to reset your password: {resetUrl}
    ...
    )
    {
        ...
        {
            var appUrl = _configuration["FrontendUrl"] ?? "http://localhost:3000";
            var confirmUrl = $"{appUrl}/confirm-email-change?token={confirmationToken}";
            var fromEmail = _configuration["Email:FromAddress"] ?? "onboarding@resend.dev";
            ...
            var message = new EmailMessage
            {
                From = fromEmail,
                To = new[] { toEmail },
                Subject = "Confirm your new email address - SkillsBarter",
                TextBody =
                    $@"Hello {userName},

                    You requested to change your email address to this email. Click the link below to confirm:
                    {confirmUrl}

                    This link will expire in 24 hours. If you didn't request this change, please ignore this email.

                    For security reasons, your old email address will remain active until you confirm this change.",
            };

            await _resend.EmailSendAsync(message);
            _logger.LogInformation($"Email change confirmation sent to {toEmail}");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, $"Failed to send email change confirmation to {toEmail}");
            throw;
        }
    }
}
